...
def train(num_epochs, initial_eta, data_fp, split, delay_g_training = 1, params_file = None):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(data_fp, split, batch_size, shuffle=False):
            ...
            # for now - save time on training, train the minimum possible batches
            if train_batches == delay_g_training:
                break
...
def main(data_file, num_epochs=100, initial_eta=2e-4, delay_g_training = 1, params_file = None):
    
    print("Loading data...")
    with h5py.File(data_file,'r') as hf:
        train(num_epochs, initial_eta, hf, 'train2014', delay_g_training, params_file)
...
if __name__ == '__main__':
    ...
    main(args.data_file, num_epochs=args.num_epochs, 
         params_file=args.params_dir, delay_g_training=args.delay_g_training)
